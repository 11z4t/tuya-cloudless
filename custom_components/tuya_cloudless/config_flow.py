...
# Local keys are exactly 16 alphanumeric ASCII characters.  The pairing server
# generates lowercase hex keys (secrets.token_hex(8)), but keys retrieved from
# the Tuya IoT Platform or third-party tools may contain uppercase letters or
# non-hex alphanumeric characters.  We accept any 16-char [0-9a-zA-Z] string
# to avoid blocking legitimate keys while still rejecting control chars, spaces,
# and special characters that could cause issues downstream.
_LOCAL_KEY_RE = re.compile(r"^[0-9a-zA-Z]{16}$")
...
class TuyaCloudlessConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_pair(self, user_input: dict[str, Any] | None = None) -> ConfigFlowResult:
        ...
        if user_input is not None:
            ...
            if (
                ...
                and _LOCAL_KEY_RE.match(local_key)
    ...
    async def async_step_local_key(
        ...
        if user_input is not None:
            ...
            elif not _LOCAL_KEY_RE.match(local_key):
